...
t_a_rollback_can_take_the_lock_after_an_earlier_run_left_processes_behind() {
  ...
  ql_lock converge-lock-model
  eq "$(head -n1 "$(_ql_state_dir converge-lock-model)/lock.d/owner")" "$(_ql_owner_record)" \
...
t_a_crashed_run_does_not_block_the_lock_for_ever() {
  ...
  err=$T/takeover.err
  ql_lock converge-lock-model 2>"$err" # a ql_die here would end the test, which is exactly the 1.4.0 behaviour
...
t_a_second_live_run_is_still_refused() {
  ...
  dir=$(_ql_state_dir converge-lock-model)/lock.d
  # shellcheck disable=SC2016 # the child shell expands these, not this one
  bash -c '. "$1/scripts/lib/quadlet-lib.sh"; ql_lock converge-lock-model; sleep 30' _ "$REPO" &
  holder=$!
  ...
  # shellcheck disable=SC2016 # the child shell expands these, not this one
  expect_fail env -u QL_LOCK_HELD bash -c '. "$1/scripts/lib/quadlet-lib.sh"; ql_lock converge-lock-model' _ "$REPO"
  has "$OUT" "another install/upgrade/uninstall of converge-lock-model is running"
...
t_a_child_script_reuses_the_lock_its_caller_holds() {
  ...
  # shellcheck disable=SC2016 # the child shell expands these, not this one
  expect_ok bash -c '. "$1/scripts/lib/quadlet-lib.sh"; ql_lock converge-lock-model' _ "$REPO"
  has "$OUT" "keeping the lock held by the calling script"
  [[ -d $(_ql_state_dir converge-lock-model)/lock.d ]] || die_t "the child released the lock its caller holds"
  return 0
}
